...
var disText
func _ready():
	disText = FastNoiseLite.new()
	player.color = colorNames.pick_random()
	colorNames.erase(player.color)
...
func _on_button_pressed():
	...
	if cards.find(c) != -1:
		$"../%Log".addDeathMessage(player.weapon, player, c.player)
		c.disentigrate()
...
func announceDisentigration():
	print(player.fName + " says YEEEOUCH")
